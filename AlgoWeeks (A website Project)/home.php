...
<div class="row">
  <div class="leftcolumn">
    <div class="card">
      <form action="" method="POST">
        ...
        <input type="radio" name=type value="Algorithm & Concepts"> Algorithm & Concepts
        <input type="radio" name=type value="Code"> Code<br><br>
      ...
      <?php
        ...
        if(isset($_POST['submit'])){
          ...
          if(!isset($_POST['type']) || $head=='' || $post1==''){
            echo "All fields must be filled up!";
          }
          else{
            $type= $_POST['type'];
            $sql = "INSERT INTO blog (user, post, name, type) VALUES('$user','$post1','$head','$type');";
            if ($link->query($sql) === TRUE) {
              echo '<script language="javascript">';
              echo 'window.alert("your blog posted successfully!")';
              echo '</script>';
            } 
            else {
              echo "Error posting your blog: " . $link->error;
              echo "<br>";
            }
          }
        } 
    ...
    <div class="card">
      <?php
        $user = $_SESSION['sess_user'];
        require('C:\xampp\htdocs\AlgoWeeks\connection.php');
        $sql = "SELECT id, user, post, type, name, time FROM blog ORDER BY time desc;";
        $com="";
        $comid=0;
          ...
          if (mysqli_num_rows($result) > 0) {
              while($row = mysqli_fetch_array($result)) {
                ...
                <form action="home.php" method="POST">
                  ...
                  <input id='submitid' type='submit' name="<?php echo $id?>" value='Comment'>
                </form>
                <?php
                if(isset($_POST["$id"])){
                  $com = $_POST['comment'];
  ...
  <div class="card">
      ...
      <?php
        ...
        $user = $_SESSION['sess_user'];
        require('C:\xampp\htdocs\AlgoWeeks\connection.php');
        $sql = "SELECT blogid, COUNT(blogid) FROM comments GROUP BY blogid desc;";
        if($result = mysqli_query($link,$sql)){
          $cnt=0;
          if (mysqli_num_rows($result) > 0) {
              while($row = mysqli_fetch_array($result)) {
        ...
        $sql = "SELECT name FROM blog WHERE id='$blogid';";
        if($result = mysqli_query($link,$sql)){
          if (mysqli_num_rows($result) > 0) {
              while($row = mysqli_fetch_array($result)) {
                echo "<div class='fakeimg'>".$row['name']."</div>";
              }
          } 
        }
      ?>
